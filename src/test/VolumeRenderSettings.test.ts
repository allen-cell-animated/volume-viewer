...
import { Axis, VolumeRenderSettingUtils, defaultVolumeRenderSettings } from "../VolumeRenderSettings";
...
describe("VolumeRenderSettingUtils", () => {
  ...
  it("deeply-copies bounds", () => {
    const s1 = defaultVolumeRenderSettings(volume);
  ...
  it("can compare settings objects", () => {
    const s1 = defaultVolumeRenderSettings(volume);
    const s2 = defaultVolumeRenderSettings(volume);
    ...
    const s3 = defaultVolumeRenderSettings(volume);
    s3.specular[0][1] = 15;
    expect(VolumeRenderSettingUtils.isEqual(s1, s3)).to.be.false;

    // Change bounds
    const s4 = defaultVolumeRenderSettings(volume);
